...
public class TestHelpers
{
    ...
    public static Compilation GetInputCompilation<TGenerator>(OutputKind outputKind, string[]? explicitUsings, params string[] code)
        => GetInputCompilation<TGenerator>(outputKind, explicitUsings, code.Select(x => CSharpSyntaxTree.ParseText(x)).ToArray());
    public static Compilation GetInputCompilation<TGenerator>(OutputKind outputKind, string[]? explicitUsings, params SyntaxTree[] syntaxTrees)
    {
        explicitUsings ??= commonUsings;

        // Create the initial syntax tree, add using statements, and get the updated tree
        var newUsings = explicitUsings.Select(x => SyntaxFactory.UsingDirective(SyntaxFactory.ParseName(x))).ToArray();
    ...
    public static (ISymbol? symbol, CancellationToken cancellationToken, IEnumerable<Diagnostic> inputDiagnostics)
        GetSymbolForSynatax<T>(string sourceCode, Func<T, bool>? filter = null, bool continueOnInputErrors = false)
        where T : SyntaxNode
    {
        ...
        var compilation = TestHelpers.GetInputCompilation<Generator>(
                OutputKind.DynamicallyLinkedLibrary, null, sourceCode);
        var inputDiagnostics = compilation.GetDiagnostics();
        if (!continueOnInputErrors && TestHelpers.WarningAndErrors(inputDiagnostics).Any())
        { return (null, cancellationToken, inputDiagnostics); }
        ...
        return (semanticModel.GetDeclaredSymbol(syntaxNode), cancellationToken, inputDiagnostics);
    }
        ...
        where T : IIncrementalGenerator, new()
    {
        ...
        var inputCompilation = GetInputCompilation<T>(outputKind, null, syntaxTrees);
